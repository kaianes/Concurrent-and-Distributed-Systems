...
public class Car implements Runnable{
	...
	if (cartype==REDCAR) {
            if (inFront.getX() - xpos > 100) {
                xpos += 4;
                if (xpos >= bridgeXLeft & xposOld < bridgeXLeft) controller.redEnterLeft();
		...
		else if (xpos >= bridgeXRight &&  xposOld < bridgeXRight) controller.redLeaveRight();
            }
	} else {
            if (xpos-inFront.getX() > 100) {
                xpos -= 4;
                if (xpos <= bridgeXRight && xposOld > bridgeXRight) controller.blueEnterRight();
		...
		else if (xpos <= bridgeXLeft && xposOld > bridgeXLeft) controller.blueLeaveLeft();
            }
	}
    }
